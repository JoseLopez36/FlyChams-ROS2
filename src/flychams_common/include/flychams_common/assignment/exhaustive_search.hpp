...
namespace flychams::common
{
    /**
     * ════════════════════════════════════════════════════════════════
     * @brief Solver for agent assignment using exhaustive search
     *
     * @details
     * Explores every leaf of the assignment tree without pruning.
     * Every permutation is evaluated regardless of the partial cost,
     * guaranteeing the true global optimum and providing a reference
     * baseline to benchmark Branch-and-Bound pruning efficiency.
     * ════════════════════════════════════════════════════════════════
    ...
    private: // Recursive cost calculation. Exhaustive search
            ...
            std::vector<PositionSolver::SharedPtr>& solvers, int& node_count)
        {
            ...
            // in the same order as the input agent array
            {
                ...
                for (int p = 0; p < n_perms; p++)
                {
                    ...
                    // Recursive call — always descend, no bound check
        ...
        void agentCost(Agent& Ak, const std::vector<Cluster>& Tk,
            ...
            const int& nk, PositionSolver::SharedPtr solver)
        {
