...
def generate_launch_description():
    ...
    config_dir = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            get_package_share_directory('flychams_coordinator'), 'config'),
        description='Path to coordinator configuration directory'
    )

    # Load launch configuration
    launch_config_path = os.path.join(
        get_package_share_directory('flychams_coordinator'), 'config', 'launch.yaml')
